...
function insertProduit(responce){
    ...
    for(let i=0 ; i<responce.length; i++){
        articleProduit.innerHTML += '<article id="produit">' + '<div class="main_produit">' +'<div class="image">'+ '<section class="produit_item">' +'<div class="img_produit">' + '<img src="'+responce[i].imageUrl +'" >' + '</div>'+ '<div class="nom_produit">'+ '<h3>'+responce[i].name+'</h3>'+'</div>' + '<div class="prix-produit">'+ '<p>'+ responce[i].price/100 +'€</p>'+ '</div>'+'<div>'+ '<a href="pages/Page%20article.html?id='+ responce[i]._id +'">'+ 'voir la fiche produit'+ '</a>'+'</div>' +'</section>' +'</div>' + '</div>' + '</article>';
    }
}
